# ARCHIVED — the hand-maintained STATUS, frozen 2026-09-09

Superseded by `TUF/STATUS.md`, which is now generated from the progress
database (ADR-0004 phase C). Kept, not deleted, because it carries measured
detail that the database does not model: the case-by-case sandbox verification
table, the effective-memory-bandwidth derivations, and the "corrections to the
pack" narrative.

**Do not update this file.** Live state is `trainwatch curriculum`. The parts
of it that were structured facts — phases, gates, decisions, open questions —
were transcribed into `configs/curriculum.yaml`; where a claim here disagrees
with the generated STATUS.md, this file is the stale one. Two of its claims were
already false when it was frozen: Python 3.14 as a settled decision (reversed
to 3.12 in `bdfa92b`), and "nothing measured on a model yet" (contradicted by
`results/serving-tuf.json` and `results/train-tuf.json`).
